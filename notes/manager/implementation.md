# ComfyUI Manager Integration - Implementation Guide

**Date:** 2025-10-20  
**Project:** fl_js  
**Parent Documents:** [research.md](./research.md), [investigation.md](./investigation.md)  
**Purpose:** Complete implementation with full code blocks
...
1. **`backend/comfy_manager.py`** - NEW FILE (~600 lines)
2. **`backend/mcp_server.py`** - MODIFICATIONS (~400 lines added)

### Implementation Steps

1. Create `backend/comfy_manager.py` with complete client
2. Modify `backend/mcp_server.py` imports
3. Add request models to `backend/mcp_server.py`
4. Add Manager detection to lifespan
5. Add tool definitions to `backend/mcp_server.py`
6. Test
...
**File:** `backend/comfy_manager.py` (NEW)
...
**Location:** After line ~30 (after existing imports)
...
## Step 3: Modify backend/mcp_server.py - Request Models

**File:** `backend/mcp_server.py`  
**Location:** After line ~300 (after existing request models, before tools section)
...
## Step 4: Modify backend/mcp_server.py - Lifespan (Optional Detection)

**File:** `backend/mcp_server.py`  
**Location:** In `mcp_lifespan` function (around line 172-210)

**Note:** This step is OPTIONAL. The tools will work without it - they handle Manager detection internally. This just provides early detection and logging.

**Add after the subprocess/standalone logic, before the final yield:**

```python
    # Check if ComfyUI Manager is installed (optional early detection)
    try:
...
**File:** `backend/mcp_server.py`  
**Location:** After line ~1000 (after existing tools, before the end of file)
...
1. **`backend/comfy_manager.py`** (~600 lines)
   - Complete Manager client implementation
   - Exception classes
   - Data classes
   - Cache system
...
1. **`backend/mcp_server.py`** (~400 lines added)
   - **Line ~30:** Import Manager client and types
   - **Line ~300:** Add 6 request model classes
   - **Line ~180 (optional):** Add Manager detection to lifespan
   - **Line ~1000:** Add 6 tool definitions
...
ls -la backend/comfy_manager.py

# Check imports work
python -c "from backend.comfy_manager import get_comfy_manager_client; print('OK')"
```

### 2. Test Manager Detection

```python
# In Python REPL or test script
import asyncio
from backend.comfy_manager import get_comfy_manager_client

async def test_detection():
    client = get_comfy_manager_client()
    version = await client.check_installed()
...
# - manager_check_installed
# - manager_search_nodes with query="image"
...
```bash
# Stop ComfyUI server
# Call any manager tool
# Should return connection_error, not crash

# Point to ComfyUI without Manager installed
# Should return not_installed error
```
...
1. **Test with real ComfyUI instance**
2. **Verify all 6 tools work correctly**
3. **Check error handling paths**
4. **Update project documentation**
5. **Consider adding to README**
6. **Future: Add write operations (install/update/uninstall)**
