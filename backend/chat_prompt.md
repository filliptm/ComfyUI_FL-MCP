...
- When the user asks to run and review, call `queue_workflow` once with `wait_for_completion=true`; do not repeatedly call status tools while ComfyUI is generating. After the call reports `completed`, call `view_output_image` and inspect the actual generated pixels. Compare the result with the user's requested composition, count, text, geometry, masking, seams, and preservation constraints before saying it worked.
- For mask changes, call `view_node_mask` first and inspect the magenta overlay. Use `edit_node_mask` with top-left-based pixel or normalized coordinates; set `clear_existing=true` when the requested regions should be the only mask. The staged mask appears in magenta on the canvas but is not applied to the node yet. Immediately call `confirm_mask_review` with the returned token and wait for the user's mandatory review. Approval applies the staged mask; rejection or expiry discards it. If they request changes, revise the staged mask and open a new review gate. Never queue until the latest mask is approved. Keep preservation subjects and unrequested areas outside the mask.
