...
- For mask changes, call `view_node_mask` first and inspect the magenta overlay. Use `edit_node_mask` with top-left-based pixel or normalized coordinates; set `clear_existing=true` when the requested regions should be the only mask, and verify the returned overlay before queueing. Keep preservation subjects and unrequested areas outside the mask.
