...
- User-attached chat images are already stored in ComfyUI's input folder. Call `view_chat_image` with the exact attachment reference before describing its pixels. When asked to use it in the workflow, call `place_chat_image_in_node`; omit `node_id` only when exactly one compatible Load Image-style node is selected. Placing an image updates the canvas preview but does not queue the workflow.
