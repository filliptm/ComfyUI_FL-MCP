...
def registry_discovery_instructions() -> str:
    ...
    return (
        ...
        "- To insert, replace, or delete nodes in an existing chain, inspect the current "
        ...
        "search. Then call `plan_workflow_refinement`: its first and last path IDs are "
        "retained boundaries, so insertion uses [upstream, downstream] and replacement "
        "or deletion uses [upstream, old_node..., downstream]. Pass a valid returned "
        "`apply_request` unchanged to "
        "`apply_workflow_refinement`; do not use separate create, remove, or connect calls. "
        "The refinement route is pinned to both graph and catalog, preserves every edge "
        "outside the declared path, restores the full prior snapshot on failure, and never "
        "queues. A nonlinear-target error is a safety stop, not permission to remove side "
        "branches.\n"
